...
async function loadHeader() {
    try {
        const response = await fetch('../store-app/header.html');
        if (!response.ok) throw new Error('Network response was not ok: ' + response.statusText);
        const headerHtml = await response.text();
        const headerPlaceholder = document.getElementById('header-placeholder');
        if (headerPlaceholder) { headerPlaceholder.innerHTML = headerHtml; } 
        else { console.error("Error: Header placeholder div not found."); }
    ...
    modal.innerHTML = `
        <div class="modal-content cart-modal">
            ...
            <div class="modal-body">
                ${cartIsEmpty ? '<p class="empty-cart">Your cart is empty. Start shopping!</p>' :
                    `<div class="cart-items">${cart.items.map(item => `
                        ...
                        `).join('')}</div>
                    <div class="cart-summary"><div class="cart-total"><strong>Total:</strong><strong>KSH ${cart.getTotal().toLocaleString()}</strong></div></div>`
                }
            </div>
...
function proceedToCheckout() {
    ...
    setTimeout(() => { if (confirm('Your order has been sent via WhatsApp. Would you like to clear your cart?')) { cart.clear(); document.querySelector('.modal-overlay').remove(); document.body.style.overflow = 'auto'; } }, 1000);
}
